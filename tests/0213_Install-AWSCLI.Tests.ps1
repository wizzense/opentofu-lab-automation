# filepath: tests/0213_Install-AWSCLI.Tests.ps1
...
BeforeAll {
    ...
    # Get the script path using the LabRunner function  
        $script:ScriptPath = Get-RunnerScriptPath '0213_Install-AWSCLI.ps1'
        if (-not $script:ScriptPath -or -not (Test-Path $script:ScriptPath)) {
            throw "Script under test not found: 0213_Install-AWSCLI.ps1 (resolved path: $script:ScriptPath)"
        }
...
Describe '0213_Install-AWSCLI Tests' -Tag 'Installer' {
    ...
    Context 'Script Structure Validation' {
        ...
        It 'should define expected functions' -Skip:($SkipNonWindows) {
            Get-Command 'Install-AWSCLI' -ErrorAction SilentlyContinue | Should -Not -BeNullOrEmpty
        }
    }
    ...
    Context 'Install-AWSCLI Function Tests' {
        It 'should be defined and accessible' -Skip:($SkipNonWindows) {
            Get-Command 'Install-AWSCLI' | Should -Not -BeNullOrEmpty
        }
                It 'should support common parameters' -Skip:($SkipNonWindows) {
            (Get-Command 'Install-AWSCLI').Parameters.Keys | Should -Contain 'Verbose'
            (Get-Command 'Install-AWSCLI').Parameters.Keys | Should -Contain 'WhatIf'
        }
                It 'should handle execution with valid parameters' -Skip:($SkipNonWindows) {
            # Add specific test logic for Install-AWSCLI
            $true | Should -BeTrue  # Placeholder - implement actual tests
        }
    }
}
