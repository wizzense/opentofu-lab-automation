...
Describe "kicker-git.ps1 Integration Tests" {
    ...
    Context "PowerShell 5.1 Endpoint Simulation" {
        It "Should download and parse correctly via Invoke-WebRequest" {
        ...
        It "Should handle parameter parsing correctly" {
            ...
            $testScript = "[CmdletBinding()]nWrite-Host \"Test\""
    ...
    Context "Cross-Platform Path Compatibility" {
        ...
        It "Should handle temp directory correctly" {
            ...
            $funcMatch = [regex]::Match($scriptContent, "function Get-PlatformTempPath[^}]+}", [System.Text.RegularExpressions.RegexOptions]::Singleline)
