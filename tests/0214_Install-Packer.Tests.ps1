...
BeforeAll {
    ...
    $scriptPath = Join-Path $PSScriptRoot '..' '/workspaces/opentofu-lab-automation/pwsh/runner_scripts/0214_Install-Packer.ps1'
...
Describe '0214_Install-Packer Tests' -Tag 'Installer' {
    ...
    Context 'Script Structure Validation' {
        ...
        It 'should define expected functions' -Skip:($SkipNonWindows) {
            Get-Command 'Install-Packer' -ErrorAction SilentlyContinue | Should -Not -BeNullOrEmpty
        }
    }
    ...
    Context 'Install-Packer Function Tests' {
        It 'should be defined and accessible' -Skip:($SkipNonWindows) {
            Get-Command 'Install-Packer' | Should -Not -BeNullOrEmpty
        }
                It 'should support common parameters' -Skip:($SkipNonWindows) {
            (Get-Command 'Install-Packer').Parameters.Keys | Should -Contain 'Verbose'
            (Get-Command 'Install-Packer').Parameters.Keys | Should -Contain 'WhatIf'
        }
