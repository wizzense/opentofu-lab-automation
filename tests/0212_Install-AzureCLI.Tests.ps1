# filepath: tests/0212_Install-AzureCLI.Tests.ps1
...
BeforeAll {
    ...
    # Get the script path using the LabRunner function  
        $script:ScriptPath = Get-RunnerScriptPath '0212_Install-AzureCLI.ps1'
        if (-not $script:ScriptPath -or -not (Test-Path $script:ScriptPath)) {
            throw "Script under test not found: 0212_Install-AzureCLI.ps1 (resolved path: $script:ScriptPath)"
        }
...
Describe '0212_Install-AzureCLI Tests' -Tag 'Installer' {
    ...
    Context 'Script Structure Validation' {
        ...
        It 'should define expected functions' -Skip:($SkipNonWindows) {
            Get-Command 'Install-AzureCLI' -ErrorAction SilentlyContinue | Should -Not -BeNullOrEmpty
        }
    }
    ...
    Context 'Install-AzureCLI Function Tests' {
        It 'should be defined and accessible' -Skip:($SkipNonWindows) {
            ...
            (Get-Command 'Install-AzureCLI').Parameters.Keys | Should -Contain 'Verbose'
            (Get-Command 'Install-AzureCLI').Parameters.Keys | Should -Contain 'WhatIf'
        }
                It 'should handle execution with valid parameters' -Skip:($SkipNonWindows) {
            # Add specific test logic for Install-AzureCLI
            $true | Should -BeTrue  # Placeholder - implement actual tests
        }
    }
}
