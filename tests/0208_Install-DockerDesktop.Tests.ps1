...
BeforeAll {
    ...
    # Get the script path using the LabRunner function  
        $script:ScriptPath = Get-RunnerScriptPath '0208_Install-DockerDesktop.ps1'
        if (-not $script:ScriptPath -or -not (Test-Path $script:ScriptPath)) {
            throw "Script under test not found: 0208_Install-DockerDesktop.ps1 (resolved path: $script:ScriptPath)"
        }
