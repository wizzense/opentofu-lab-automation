...
# filepath: tests/0111_Disable-TCPIP6.Tests.ps1
...
BeforeAll {
    ...
    # Get the script path using the LabRunner function  
        $script:ScriptPath = Get-RunnerScriptPath '0111_Disable-TCPIP6.ps1'
        if (-not $script:ScriptPath -or -not (Test-Path $script:ScriptPath)) {
            throw "Script under test not found: 0111_Disable-TCPIP6.ps1 (resolved path: $script:ScriptPath)"
        }
...
Describe '0111_Disable-TCPIP6 Tests' -Tag 'Feature' {
