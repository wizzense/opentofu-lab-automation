# filepath: tests/0104_Install-CA.Tests.ps1
...
BeforeAll {
    ...
    # Get the script path using the LabRunner function  
        $script:ScriptPath = Get-RunnerScriptPath '0104_Install-CA.ps1'
        if (-not $script:ScriptPath -or -not (Test-Path $script:ScriptPath)) {
            throw "Script under test not found: 0104_Install-CA.ps1 (resolved path: $script:ScriptPath)"
        }
...
Describe '0104_Install-CA Tests' -Tag 'Installer' {
