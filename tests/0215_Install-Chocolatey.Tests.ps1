# filepath: tests/0215_Install-Chocolatey.Tests.ps1
...
BeforeAll {
    ...
    # Get the script path using the LabRunner function  
        $script:ScriptPath = Get-RunnerScriptPath '0215_Install-Chocolatey.ps1'
        if (-not $script:ScriptPath -or -not (Test-Path $script:ScriptPath)) {
            throw "Script under test not found: 0215_Install-Chocolatey.ps1 (resolved path: $script:ScriptPath)"
        }
...
Describe '0215_Install-Chocolatey Tests' -Tag 'Installer' {
    ...
    Context 'Install-Chocolatey Function Tests' {
        It 'should be defined and accessible' -Skip:($SkipNonWindows) {
            ...
            (Get-Command 'Install-Chocolatey').Parameters.Keys | Should -Contain 'Verbose'
            (Get-Command 'Install-Chocolatey').Parameters.Keys | Should -Contain 'WhatIf'
        }
                It 'should handle execution with valid parameters' -Skip:($SkipNonWindows) {
            # Add specific test logic for Install-Chocolatey
            $true | Should -BeTrue  # Placeholder - implement actual tests
        }
    }
}
