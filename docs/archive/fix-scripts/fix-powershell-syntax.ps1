...
foreach ($script in $scripts) {
    ...
    if ($lines.Count -ge 2 -and 
        ...
        Write-Host "   VALIDATING: Moving Import-Module after Param block" -ForegroundColor Yellow
        ...
        # DISABLED: # DISABLED: Set-Content -Path $script.FullName -Value $newContent -NoNewline
        
        Write-Host "  VALIDATION: Found issue -: $($script.Name)" -ForegroundColor Green
        $fixedCount++
...
Write-Host "VALIDATION: Found issue -: $fixedCount scripts" -ForegroundColor Green
