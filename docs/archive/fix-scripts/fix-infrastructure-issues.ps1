    ...
    foreach ($file in $problemFiles) {
        if (Test-Path $file) {
            if ($DryRun) {
                ...
                Re# VALIDATION-ONLY: Would move item $file -Force
    ...
    foreach ($file in $publicFiles) {
        try {
            ...
            if (-not $DryRun) {
                ...
                # DISABLED: Set-Content $file.FullName $fixedContent
...
"@
        ...
        if (-not $DryRun) {
            # DISABLED: Set-Content $testHelperPath $content
        }
    }
}
...
function Fix-TestContainers {
    ...
    foreach ($testFile in $testFiles) {
        try {
            ...
            # DISABLED: Set-Content $testFile.FullName $content
            $fixedCount++
        }
    }
...
function Fix-ImportPaths {
    param(bool$DryRun, bool$ValidateOnly)
    ...
    foreach ($file in $files) {
        ...
        if ($content -match 'Import-Module\s+"(?!/)') {
            ...
            if (-not $DryRun -and $ValidateOnly) {
    ...
    foreach ($workflow in $workflowFiles) {
        ...
        if ($content -ne $originalContent) {
            if ($DryRun) {
                ...
                # DISABLED: Set-Content $workflow.FullName $content
...
try {    switch ($Fix) {
        ...
        'ImportPaths' { Fix-ImportPaths -DryRun $DryRun -ValidateOnly $ValidateOnly }
        ...
        'All' {
            ...
            Fix-MissingCommands
            Fix-TestContainers
            Fix-ImportPaths -DryRun $DryRun -ValidateOnly $ValidateOnly
            Fix-GitHubActions
        }
    }
