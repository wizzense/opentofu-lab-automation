...
foreach ($testFile in $testFiles) {
    try {
        ...
        if ($changed -and -not $WhatIf) {
            # DISABLED: # DISABLED: Set-Content -Path $testFile.FullName -Value $content -Encoding UTF8
            Write-Host "   Fixed: $($testFile.Name)" -ForegroundColor Green
            $fixedCount++
        }
        elseif ($changed -and $WhatIf) {
            Write-Host "  WHATIF VALIDATION ISSUE: $($testFile.Name)" -ForegroundColor Magenta
            $fixedCount++
        }
        else {
