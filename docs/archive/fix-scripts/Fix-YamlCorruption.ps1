...
function Fix-YamlCorruption {
 ...
 content | # DISABLED: Out-File -FilePath $FilePath -Encoding UTF8 -NoNewline
 return $fixesApplied
 }
...
foreach ($file in $yamlFiles) {
 ...
 if ($fixesApplied.Count -gt 0) {
 Write-Host "VALIDATION: Found issue - $($fixesApplied.Count) corruptions in: $relativePath" -ForegroundColor Green
