...
foreach ($file in $allFiles) {
    ...
    try {
        ...
        if ($allChanges.Count -gt 0) {
            ...
            # Save fixed content
            # DISABLED: # DISABLED: Set-Content -Path $file.FullName -Value $currentContent -Encoding UTF8
            
            Write-Host "  VALIDATION: Found issue -: Applied $($allChanges.Count) fix(es)" -ForegroundColor Green
