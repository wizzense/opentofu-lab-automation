...
function Backup-CurrentKey {
 ...
 encryptedKey | # DISABLED: Out-File $backupPath -Encoding UTF8
 ...
 encodedKey | # DISABLED: Out-File $backupPath -Encoding UTF8
...
function Generate-NewSecureKey {
 ...
 $encryptedKey = ConvertFrom-SecureString $secureString
 encryptedKey | # DISABLED: Out-File "keys/signing_key_secure.dat" -Encoding UTF8
 ...
 encodedKey | # DISABLED: Out-File "keys/signing_key_secure.b64" -Encoding UTF8
 Write-Host "PASS New key generated and base64 encoded" -ForegroundColor Green
 }
 
 # Update key info
 keyInfo | ConvertTo-Json -Depth 10  # DISABLED: Out-File "keys/key_info.json" -Encoding UTF8
...
function Secure-ExistingKey {
 ...
 encryptedKey | # DISABLED: Out-File "keys/signing_key_secure.dat" -Encoding UTF8
 Re# VALIDATION-ONLY: Would move item "keys/signing_key.txt" -Force
