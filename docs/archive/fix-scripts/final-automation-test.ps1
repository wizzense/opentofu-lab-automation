...
try {
    ...
    # Cleanup temp files
    Re# VALIDATION-ONLY: Would move item "bootstrap-output.txt", "bootstrap-error.txt" -ErrorAction SilentlyContinue
} catch {
...
if ($issues.Count -gt 0) {
    Write-Host "`nWARN Issues Found:" -ForegroundColor Yellow
...
if ($passedTests -eq $totalTests) {
    ...
    Write-Host "`nWARN Some automation components need attention" -ForegroundColor Yellow
