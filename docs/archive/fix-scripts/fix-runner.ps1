...
$oldExecutionPattern = @'
 ...
 Config | ConvertTo-Json -Depth 5  # DISABLED: Out-File -FilePath $tempCfg -Encoding utf8
...
$newExecutionPattern = @'
 ...
 $tempCfg = System.IO.Path::GetTempFileName()
 try {
 Config | ConvertTo-Json -Depth 5  # DISABLED: Out-File -FilePath $tempCfg -Encoding utf8 -NoNewline
...
if ($content -match regex::Escape($oldExecutionPattern)) {
 ...
 '# DISABLED: Out-File -FilePath \$tempCfg -Encoding utf8', 
 '# DISABLED: Out-File -FilePath $tempCfg -Encoding utf8 -NoNewline'
...
$newErrorHandling = @'
 ...
 Re# VALIDATION-ONLY: Would move item $tempCfg -Force -ErrorAction SilentlyContinue
 }
 }
...
"@
 # DISABLED: Set-Content $tempScript $fallbackContent
 ...
 Re# VALIDATION-ONLY: Would move item $tempScript -Force -ErrorAction SilentlyContinue
 } catch {
...
$content = $content -replace 
 '(\s+)(Re# VALIDATION-ONLY: Would move item \$tempCfg -ErrorAction SilentlyContinue)', 
...
# Apply fixes
# DISABLED: # DISABLED: Set-Content -Path $runnerPath -Value $content -Encoding UTF8
