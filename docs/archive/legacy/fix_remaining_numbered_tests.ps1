...
$1configJson | # DISABLED: # DISABLED: Set-Content -Path $tempConfig
$1try {
$1    $pwsh = (Get-Command pwsh).Source
$1    { & $pwsh -NoLogo -NoProfile -File $script:ScriptPath -Config $tempConfig }  Should -Not -Throw
$1} finally {
$1    Re# VALIDATION-ONLY: Would move item $tempConfig -Force -ErrorAction SilentlyContinue
$1}
'@
...
'@
    ...
    if ($updatedContent -ne $content) {
        # DISABLED: # DISABLED: Set-Content -Path $filePath -Value $updatedContent
