...
if (-not (Test-Path $Source)) {
    ...
    try {
        if ($WhatIf) {
            ...
            # VALIDATION-ONLY: Would move item -Path $Source -Destination $Destination -Force:$Force
...
if (-not (Test-Path $Source)) {
    ...
    try {
        if ($WhatIf) {
            ...
            # VALIDATION-ONLY: Would copy item -Path $Source -Destination $Destination -Force:$Force
...
----------------
 ...
 auto-fix.ps1  Replaced by Invoke-ValidateOnly function 
...
if ($WhatIf) {
    ...
    try {
        # DISABLED: # DISABLED: Set-Content -Path (Join-Path $rootDir "CLEANUP-SUMMARY.md") -Value $cleanupSummary -Force
