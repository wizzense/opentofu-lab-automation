...
.PARAMETER CreateIssueForBugFix
    Automatically create a GitHub issue when applying bug fixes
    
.PARAMETER IssueLabels
    Labels to apply to automatically created GitHub issues
    
.PARAMETER IssuePriority
    Priority level for automatically created GitHub issues

.EXAMPLE
    Invoke-GitControlledPatch -PatchDescription "Fix syntax errors" -PatchOperation { Write-Host "Fixing syntax" } -CreatePullRequest

.EXAMPLE
    Invoke-GitControlledPatch -PatchDescription "Fix critical bug in validation" -PatchOperation { 
        # Fix code here 
    } -CreatePullRequest -CreateIssueForBugFix -IssuePriority "High" -IssueLabels @("bug", "critical", "automated-fix")
...
function Invoke-GitControlledPatch {
    ...
    param(
        ...
        Parameter(Mandatory = $false)
        switch$CreateIssueForBugFix,
        Parameter(Mandatory = $false)
        string$IssueLabels = @("bug", "automated-fix"),
        Parameter(Mandatory = $false)
    ...
    begin {
        ...
        Initialize-CrossPlatformEnvironment
          # INTELLIGENT BRANCH STRATEGY: Use comprehensive branching logic
        Write-Host "Analyzing optimal branch strategy..." -ForegroundColor Cyan
        $branchStrategy = Invoke-IntelligentBranchStrategy -PatchDescription $PatchDescription
        
        if (-not $branchStrategy.Success) {
            Write-Error $branchStrategy.Message
            return @{ Success = $false; Message = $branchStrategy.Message }
        }
        
        Write-Host "Branch strategy: $($branchStrategy.Action)" -ForegroundColor Green
        Write-Host "Working branch: $($branchStrategy.NewBranchName -or $branchStrategy.CurrentBranch)" -ForegroundColor Gray
        
        # Update variables based on strategy
        $targetBranchName = $branchStrategy.NewBranchName -or $branchStrategy.CurrentBranch
        $skipBranchCreation = -not $branchStrategy.NeedsBranchSwitch
        ...
        if (-not $branchName) {
            throw "Branch name is missing. Ensure branch strategy provides a valid name."
        }

        Write-Host "Validated branch name: $branchName" -ForegroundColor Green
...
"@
            ...
            if ($prResult.Success) {
                ...
                # Automatically create GitHub issue for bug fixes
                if ($CreateIssueForBugFix -or ($PatchDescription -match '\b(fixbugerrorissueproblembroken)\b')) {
                    Write-Host "Creating GitHub issue for bug fix..." -ForegroundColor Blue
                    try {
                        $issueResult = Invoke-GitHubIssueIntegration -PatchDescription $PatchDescription -PullRequestUrl $prResult.Url -AffectedFiles $changedFiles -Labels $IssueLabels -Priority $IssuePriority
                        if ($issueResult.Success) {
                            Write-Host "GitHub issue created: $($issueResult.IssueUrl)" -ForegroundColor Green
                            $prResult.IssueUrl = $issueResult.IssueUrl
                            $prResult.IssueNumber = $issueResult.IssueNumber
                        } else {
            ...
            return @{
                ...
                PullRequestNumber = if ($prResult.Url) { ($prResult.Url -split '/')-1 } else { $null }
                IssueUrl = $prResult.IssueUrl
                IssueNumber = $prResult.IssueNumber
            ...
            try {
                ...
                if ($currentBranch -eq $branchName) {
                    ...
                    $previousBranch = git reflog --pretty=format:'%gs'  
                                    ...
                                    ForEach-Object { $_.Matches0.Groups1.Value }  
...
# Add module paths to $env:PSModulePath
$modulePaths = @(
    "$PSScriptRoot/modules/LabRunner",
    "$PSScriptRoot/modules/CodeFixer",
    "$PSScriptRoot/modules/PatchManager"
)
...
foreach ($module in @("LabRunner", "CodeFixer", "PatchManager")) {
