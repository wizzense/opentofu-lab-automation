...
function Test-CriticalExclusion {
    ...
    # Properly handle both forward and backslashes at the beginning of the path
    $relativePath = $relativePath -replace '^[/\\]', ""
    
    foreach ($pattern in $script:CriticalExclusions) {
        # First try simple wildcard matching
        if ($relativePath -like $pattern) {
            return $true
        }
        
        # Then try regex with path separator normalization for complex patterns
