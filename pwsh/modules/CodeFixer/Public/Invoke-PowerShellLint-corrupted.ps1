...
try {
            ...
            Write-Host "⚠️ Parallel processing failed, falling back to sequential: $($_.Exception.Message)" -ForegroundColor Yellow
            $Parallel = $false
        }
    }
    ...
    $psaAvailable = & {
        try {
            ...
            try {
                ...
                Write-Host "❌ PSScriptAnalyzer initialization failed, using AST-only analysis" -ForegroundColor Red
                return $false
            }
        }
    }
