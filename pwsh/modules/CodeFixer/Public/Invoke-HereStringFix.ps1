...
function Invoke-HereStringFix {
    ...
    foreach ($file in $files) {
        try {
            ...
            foreach ($pattern in $patterns) {
                ...
                if ($matches.Count -gt 0) {
                    ...
                    Write-Host "  ✅ $($pattern.Description): $($matches.Count) fixes in $($file.Name)" -ForegroundColor Green
                }
            }
        ...
        if ($fileHasIssues) {
            ...
            if ((-not $WhatIf) -and $AutoFix -and ($originalContent -ne $content)) {
                ...
                Write-Host "  ⚠️ $fileFixes issues found in $($file.Name) - use -AutoFix to repair" -ForegroundColor Yellow
            }
        }
        } catch {
    ...
    if ($WhatIf) {
        ...
        Write-Host "  ⚠️  Report-only mode - use -AutoFix to make changes" -ForegroundColor Yellow
    }
