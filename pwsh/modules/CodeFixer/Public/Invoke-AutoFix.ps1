            ...
            if ($PSCmdlet.ShouldProcess($file.FullName, "Create backup and fix syntax errors")) {
                ...
                foreach ($error in $sortedErrors) {
                    ...
                    if ($fixResult.Fixed) {
                        ...
                        $hasChanges = $true
                        Write-Host "    ✅ Applied fix: $($fixResult.Description)" -ForegroundColor Green
                    } else {
                        Write-Host "    ⚠️  Could not auto-fix: $($error.Message)" -ForegroundColor Yellow
                    }
                }
                ...
                if ($hasChanges) {
                    ...
                    Write-Host "  ✅ Fixed and saved $($file.Name)" -ForegroundColor Green
                    ...
                    try {
                        ...
                        if ($verifyErrors.Count -eq 0) {
                            Write-Host "  ✅ Verification passed - no syntax errors remain" -ForegroundColor Green
                        } else {
                            Write-Host "  ⚠️  $($verifyErrors.Count) syntax error(s) remain" -ForegroundColor Yellow
                        }
                    } catch {
                        Write-Host "  ❌ Verification failed: $($_.Exception.Message)" -ForegroundColor Red
                    }
                } else {
                    Write-Host "  ⚠️  No automatic fixes could be applied" -ForegroundColor Yellow
