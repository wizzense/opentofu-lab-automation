...
function Invoke-ComprehensiveAutoFix {
    ...
    Write-Host "`n1️⃣ Fixing here-string syntax..." -ForegroundColor Yellow
    try {
        $hereStringParams = @{
            Path = $Path
    ...
    Write-Host "`n2️⃣ Fixing import paths..." -ForegroundColor Yellow
    try {
        $importParams = @{
            Path = $Path
        }
    ...
    Write-Host "`n3️⃣ Fixing ternary syntax..." -ForegroundColor Yellow
    try {
        $ternaryParams = @{
            Path = $Path
        }
    ...
    Write-Host "`n4️⃣ Fixing test syntax..." -ForegroundColor Yellow
    try {
        $testParams = @{
            Path = $Path
        }
    ...
    if ($CleanupRoot -and -not $WhatIf) {
        Write-Host "`n5️⃣ Cleaning up root directory..." -ForegroundColor Yellow
        try {
    ...
    Write-Host "`n6️⃣ Fixing script order..." -ForegroundColor Yellow
    try {
    ...
    if (-not $SkipValidation -and -not $WhatIf -and $AutoFix) {
        Write-Host "`n7️⃣ Running validation..." -ForegroundColor Yellow
        try {
    ...
    if ($WhatIf) {
        ...
        Write-Host "`n  ℹ️  Report-only mode - use -AutoFix to apply changes" -ForegroundColor Yellow
    }
