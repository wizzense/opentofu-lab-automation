...
function Invoke-AutoFixCapture {
    ...
    $fixPatterns = @{
        ...
        'ModuleImportPaths' = @{
            ...
            Replacement = 'Import-Module "$PSScriptRoot/../../pwsh/modules/LabRunner"'
        ...
        foreach ($fixName in $fixPatterns.Keys) {
            ...
            if ($content -match $fix.Pattern) {
                if ($fix.Pattern -eq 'UnicodeCharacters') {
                    ...
                    $unicodeChars = @{
                        '🚀' = '>>'; '📋' = 'Info:'; '🏠' = 'Path:'; '🔧' = 'Setup:'
                        '📦' = 'Package:'; '📁' = 'Directory:'; '🔊' = 'Volume:'
                        '❌' = 'ERROR:'; '⚠️' = 'WARNING:'; '💥' = 'CRITICAL:'; '✅' = 'OK:'
                    }
    ...
    if ($WhatIf) {
        Write-Host "`n⚠️ WhatIf Mode: No changes were applied" -ForegroundColor Yellow
...
function Export-FixPatterns {
    ...
    Write-Host "✅ Fix patterns exported to: $OutputPath" -ForegroundColor Green
}
...
function Import-FixPatterns {
    ...
    if (Test-Path $InputPath) {
        ...
        Write-Host "✅ Imported fix patterns from: $InputPath" -ForegroundColor Green
