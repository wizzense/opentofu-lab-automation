...
function Invoke-ParallelPesterTests {
    ...
    $testScriptBlock = {
        param($TestBatch, $BatchId, $Tags, $ExcludeTags)
        
        # Import required modules in each job
        ...
        foreach ($job in $finishedJobs) {
            if ($job.Id -notin ($results | ForEach-Object { $_.JobId })) {
                try {
                    ...
                    if ($result.Success) {
                        Write-Host "✅ Batch $($result.BatchId) completed: $($result.PassedCount)/$($result.TestCount) passed" -ForegroundColor Green
                    } else {
                        Write-Host "❌ Batch $($result.BatchId) failed: $($result.Error)" -ForegroundColor Red
                    }
                } catch {
                    Write-Host "⚠️  Failed to receive job result: $($_.Exception.Message)" -ForegroundColor Yellow
                    $completed++
                }
...
function Merge-TestResults {
    ...
    Write-Host "✅ Merged results saved to $OutputPath" -ForegroundColor Green
