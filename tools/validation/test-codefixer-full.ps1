...
Write-Host "`n[1/6] Testing CodeFixer module loading..." -ForegroundColor Yellow
try {
    ...
    Write-Host "  ✅ All CodeFixer functions loaded successfully" -ForegroundColor Green
} catch {
    Write-Host "  ❌ Failed to load CodeFixer functions: $($_.Exception.Message)" -ForegroundColor Red
    exit 1
}

# Test 2: PowerShell syntax error detection
Write-Host "`n[2/6] Testing PowerShell syntax error detection..." -ForegroundColor Yellow
$testResults = @()
...
if (Test-Path "$PSScriptRoot/test-syntax-errors.ps1") {
    try {
        ...
        if ($lintResults -and $lintResults.Count -gt 0) {
            Write-Host "  ✅ Detected $($lintResults.Count) syntax errors" -ForegroundColor Green
            ...
            Write-Host "  ⚠️  No syntax errors detected (expected some)" -ForegroundColor Yellow
            $testResults += [PSCustomObject]@{
                Test = "PowerShell Syntax Detection"
                Status = "WARN"
        ...
        Write-Host "  ❌ Error running PowerShell linting: $($_.Exception.Message)" -ForegroundColor Red
        $testResults += [PSCustomObject]@{
            Test = "PowerShell Syntax Detection"
            Status = "FAIL"
    ...
    Write-Host "  ⚠️  Test file not found: test-syntax-errors.ps1" -ForegroundColor Yellow
    $testResults += [PSCustomObject]@{
        Test = "PowerShell Syntax Detection"
        Status = "SKIP"
...
Write-Host "`n[3/6] Testing JSON configuration validation..." -ForegroundColor Yellow
...
if (Test-Path "$PSScriptRoot/test-config-errors.json") {
    try {
        ...
        if ($jsonResults -and $jsonResults.Count -gt 0) {
            Write-Host "  ✅ Detected $($jsonResults.Count) JSON issues" -ForegroundColor Green
            $testResults += [PSCustomObject]@{
            ...
            Write-Host "  ⚠️  No JSON issues detected (expected some)" -ForegroundColor Yellow
            $testResults += [PSCustomObject]@{
                Test = "JSON Config Validation"
                Status = "WARN"
        ...
        Write-Host "  ❌ Error running JSON validation: $($_.Exception.Message)" -ForegroundColor Red
        $testResults += [PSCustomObject]@{
            Test = "JSON Config Validation"
            Status = "FAIL"
            Details = $_.Exception.Message
    ...
    Write-Host "  ⚠️  Test file not found: test-config-errors.json" -ForegroundColor Yellow
    $testResults += [PSCustomObject]@{
        Test = "JSON Config Validation"
        Status = "SKIP"
        Details = "Test file missing"
...
Write-Host "`n[4/6] Testing real configuration files..." -ForegroundColor Yellow
...
if (Test-Path $configPath) {
    try {
        ...
        if ($errorCount -eq 0) {
            Write-Host "  ✅ All real config files are valid" -ForegroundColor Green
            $testResults += [PSCustomObject]@{
            ...
            Write-Host "  ⚠️  Found $errorCount errors in real config files" -ForegroundColor Yellow
            $testResults += [PSCustomObject]@{
                Test = "Real Config Files" 
                Status = "WARN"
        ...
        Write-Host "  ❌ Error validating real configs: $($_.Exception.Message)" -ForegroundColor Red
        $testResults += [PSCustomObject]@{
            Test = "Real Config Files"
            Status = "FAIL"
            Details = $_.Exception.Message
    ...
    Write-Host "  ⚠️  Config directory not found: $configPath" -ForegroundColor Yellow
    $testResults += [PSCustomObject]@{
        Test = "Real Config Files"
        Status = "SKIP"
...
Write-Host "`n[5/6] Testing LabRunner integration..." -ForegroundColor Yellow
...
if (Test-Path $labRunnerPath) {
    try {
        ...
        if ($labRunnerCommands.Count -gt 0) {
            Write-Host "  ✅ LabRunner module loaded with $($labRunnerCommands.Count) commands" -ForegroundColor Green
            $testResults += [PSCustomObject]@{
            ...
            Write-Host "  ⚠️  LabRunner module loaded but no commands found" -ForegroundColor Yellow
            $testResults += [PSCustomObject]@{
                Test = "LabRunner Integration"
                Status = "WARN"
        ...
        Write-Host "  ❌ Error loading LabRunner: $($_.Exception.Message)" -ForegroundColor Red
        $testResults += [PSCustomObject]@{
            Test = "LabRunner Integration"
            Status = "FAIL"
            Details = $_.Exception.Message
    ...
    Write-Host "  ⚠️  LabRunner module not found at: $labRunnerPath" -ForegroundColor Yellow
    $testResults += [PSCustomObject]@{
        Test = "LabRunner Integration"
        Status = "SKIP"
...
Write-Host "`n[6/6] Testing runner.ps1 functionality..." -ForegroundColor Yellow
...
if (Test-Path $runnerPath) {
    try {
        ...
        Write-Host "  ✅ Runner.ps1 syntax is valid" -ForegroundColor Green
        $testResults += [PSCustomObject]@{
        ...
        Write-Host "  ❌ Runner.ps1 has syntax errors: $($_.Exception.Message)" -ForegroundColor Red
        $testResults += [PSCustomObject]@{
            Test = "Runner.ps1 Functionality"
            Status = "FAIL"
            Details = $_.Exception.Message
    ...
    Write-Host "  ❌ Runner.ps1 not found at: $runnerPath" -ForegroundColor Red
    $testResults += [PSCustomObject]@{
        Test = "Runner.ps1 Functionality"
        Status = "FAIL"
...
Write-Host "⚠️  WARNINGS: $warnCount" -ForegroundColor $$(if (warnCount -gt 0) { 'Yellow' } else { 'Green' })
