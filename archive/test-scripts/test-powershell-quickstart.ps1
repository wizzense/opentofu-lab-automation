...
try {
    ...
    if (Test-Path "test-deploy.py") {
        Write-Host "✅ SUCCESS: deploy.py downloaded successfully" -ForegroundColor Green
        $fileSize = (Get-Item "test-deploy.py").Length
        Write-Host "   File size: $fileSize bytes" -ForegroundColor Gray
    } else {
        Write-Host "❌ FAILED: deploy.py not found after download" -ForegroundColor Red
    }
} catch {
...
try {
    ...
    if (Test-Path "test-gui.py") {
        Write-Host "✅ SUCCESS: gui.py downloaded successfully" -ForegroundColor Green
        $fileSize = (Get-Item "test-gui.py").Length
        Write-Host "   File size: $fileSize bytes" -ForegroundColor Gray
    } else {
        Write-Host "❌ FAILED: gui.py not found after download" -ForegroundColor Red
    }
} catch {
    Write-Host "❌ FAILED: $($_.Exception.Message)" -ForegroundColor Red
}
...
try {
    ...
    if ($content -match "OpenTofu Lab Automation") {
        Write-Host "✅ SUCCESS: One-liner download works" -ForegroundColor Green
        Write-Host "   Content preview: $($content.Substring(0, 100))..." -ForegroundColor Gray
    } else {
        Write-Host "❌ FAILED: Content doesn't match expected format" -ForegroundColor Red
    }
} catch {
    Write-Host "❌ FAILED: $($_.Exception.Message)" -ForegroundColor Red
}
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✅ SUCCESS: Python is available" -ForegroundColor Green
        ...
        if (Test-Path "test-deploy.py") {
            ...
            if ($helpOutput -match "OpenTofu Lab Automation") {
                Write-Host "✅ SUCCESS: deploy.py help works" -ForegroundColor Green
            } else {
                Write-Host "⚠️  WARNING: deploy.py help output unexpected" -ForegroundColor Yellow
        ...
        try {
            ...
            if ($LASTEXITCODE -eq 0) {
                Write-Host "✅ SUCCESS: Python available via 'py' command" -ForegroundColor Green
                Write-Host "   Version: $pyVersion" -ForegroundColor Gray
            }
        } catch {
            Write-Host "❌ FAILED: Neither 'python' nor 'py' commands work" -ForegroundColor Red
        }
    }
} catch {
    Write-Host "❌ FAILED: $($_.Exception.Message)" -ForegroundColor Red
...
try {
    ...
    if ($deployBatExists -and $guiBatExists) {
        Write-Host "✅ SUCCESS: Windows batch files downloaded successfully" -ForegroundColor Green
    } else {
        Write-Host "❌ FAILED: Some batch files missing" -ForegroundColor Red
...
Write-Host "===============" -ForegroundColor Cyan
Write-Host "✅ All download URLs are working correctly" -ForegroundColor Green
Write-Host "✅ PowerShell Invoke-WebRequest commands work as expected" -ForegroundColor Green
Write-Host "✅ One-liner download syntax is correct" -ForegroundColor Green
