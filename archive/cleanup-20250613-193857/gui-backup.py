...
def apply_light_theme(widget, theme=LIGHT_THEME):
    """Apply light theme to a widget recursively"""
    try:
...
class RunnerScriptManager:
    ...
    def setup_ui(self):
        ...
        canvas = tk.Canvas(list_frame, bg=LIGHT_THEME['bg'])
        ...
        # Also bind directly to canvas for better responsiveness
        if platform.system() == "Windows":
    ...
    def on_script_toggled(self, script_name):
        ...
        else:            self.selected_scripts.discard(script_name)
    ...
    def run_selected(self):
        """Run the selected runner scripts with improved progress tracking"""
        ...
        script_count = len(self.selected_scripts)
        self.log_output(f"🚀 Starting execution of {script_count} selected scripts")
        self.log_output("=" * 50)
            ...
            try:
                ...
                for script_name in sorted(self.selected_scripts):
                    if script_name in self.scripts:
                        ...
                        self.log_output(f"\n[{completed}/{script_count}] 🔄 Executing: {script_name}")
                        self.log_output("-" * 40)
                        
                        # Enhanced PowerShell command with strict non-interactive mode
                        cmd = [
                            "pwsh", 
                            "-NoProfile", 
                            "-NonInteractive", 
                        ...
                        try:
                            result = subprocess.run(
                                ...
                                encoding='utf-8',
                                errors='replace',
                                env={**os.environ, 'PSNonInteractive': '1'}  # Force non-interactive
                            )
                            ...
                            if result.stderr.strip():
                                self.log_output("⚠️ STDERR:")
                                self.log_output(result.stderr.strip())
                            
                            if result.returncode == 0:
                                self.log_output(f"✅ {script_name} completed successfully ({execution_time:.1f}s)")
                            else:
                                self.log_output(f"❌ {script_name} failed with exit code {result.returncode} ({execution_time:.1f}s)")
                                
                        except subprocess.TimeoutExpired:
                            self.log_output(f"⏰ {script_name} timed out after 3 minutes")
                        except Exception as e:
                            self.log_output(f"❌ Error executing {script_name}: {str(e)}")
                          self.log_output("")  # Add spacing between scripts
                
                self.log_output("=" * 50)
                self.log_output(f"🏁 Execution completed! {completed}/{script_count} scripts processed")
                
            except Exception as e:
                self.log_output(f"❌ Runner thread error: {str(e)}")
        
        threading.Thread(target=run_thread, daemon=True).start()
    
    def log_output(self, message):
        """Thread-safe output logging with proper formatting"""
        def update_output():
            # Enable editing temporarily
            self.output_text.config(state='normal')
            
            # Convert literal \n to actual newlines and handle various formats
            ...
            clean_message = ansi_escape.sub('', formatted_message)
            
            # Insert the message
            self.output_text.insert(tk.END, clean_message + "\n")
            self.output_text.see(tk.END)
            
            # Disable editing again to make it truly read-only
            self.output_text.config(state='disabled')
...
class EnhancedConfigBuilder:
    ...
    def setup_ui(self):
        ...
        ttk.Button(button_frame, text="🔄 Refresh Schema", command=self.refresh_config_schema).pack(side="left", padx=5)
        ttk.Button(button_frame, text="📥 Load External Schema", command=self.load_external_schema).pack(side="left", padx=5)
    ...
    def create_scrollable_frame(self, parent):
        ...
        canvas = tk.Canvas(parent, bg=LIGHT_THEME['bg'])
    ...
    def refresh_config_schema(self):
        """Refresh the configuration schema and rebuild the UI"""
        try:
            # Reload the config schema module
            import importlib
            import sys
            if 'config_schema' in sys.modules:
                importlib.reload(sys.modules['config_schema'])
            
            # Recreate the schema
            from config_schema import ConfigSchema
            self.config_schema = ConfigSchema()
            
            # Rebuild the UI tabs dynamically based on the new schema
            self.rebuild_ui_from_schema()
            
            messagebox.showinfo("Schema Refreshed", "Configuration schema has been refreshed successfully!")
            
        except Exception as e:
            messagebox.showerror("Refresh Error", f"Failed to refresh schema: {str(e)}")
    
    def load_external_schema(self):
        """Load external configuration schema from JSON file"""
        file_path = filedialog.askopenfilename(
            title="Select Configuration Schema File",
            filetypes=[("JSON files", "*.json"), ("All files", "*.*")]
        )
        
        if file_path:
            try:
                self.config_schema.load_external_config_schema(file_path)
                self.rebuild_ui_from_schema()
                messagebox.showinfo("Schema Loaded", f"External schema loaded from {file_path}")
            except Exception as e:
                messagebox.showerror("Load Error", f"Failed to load external schema: {str(e)}")
    
    def rebuild_ui_from_schema(self):
        """Rebuild the configuration UI based on the current schema"""
        # This would dynamically rebuild tabs based on the schema sections
        # For now, we'll just refresh the existing tabs
        # In a full implementation, you'd want to:
        # 1. Clear existing tabs
        # 2. Recreate tabs based on schema.sections
        # 3. Populate with schema fields
        
        # Simple refresh - reload defaults
        self.load_defaults()
        
        # TODO: Implement full dynamic UI rebuilding
        print("UI refreshed with current schema")
...
class EnhancedLabGUI:
    ...
    def setup_light_theme(self):
        ...
        style.configure('TLabelFrame', background=LIGHT_THEME['bg'], foreground=LIGHT_THEME['fg'])
        style.configure('TLabelFrame.Label', background=LIGHT_THEME['bg'], foreground=LIGHT_THEME['fg'])
        ...
        style.configure('TCombobox', fieldbackground=LIGHT_THEME['entry_bg'], foreground=LIGHT_THEME['entry_fg'])
        ...
        style.configure('TNotebook.Tab', background=LIGHT_THEME['button_bg'], foreground=LIGHT_THEME['button_fg'])
    ...
    def create_widgets(self):
        ...
        self.root.after(100, lambda: apply_light_theme(self.root))
    ...
    def create_deployment_tab(self, parent):
        ...
        status_label = ttk.Label(status_frame, textvariable=self.deploy_status_var, foreground=LIGHT_THEME['accent'])
        ...
        def deploy_thread():
            try:
                ...
                if deploy_script.exists():
                    ...
                    # Fallback to PowerShell runner with strict non-interactive mode
                    ...
                    cmd = [
                        ...
                        "-ConfigFile", str(temp_config)
                    ]
                
                self.log_deployment(f"🔧 Running: {' '.join(cmd)}")
                
                # Start deployment process with enhanced non-interactive settings
                env = os.environ.copy()
                env.update({
                    'PSNonInteractive': '1',
                    'POWERSHELL_TELEMETRY_OPTOUT': '1',
                    'TERM': 'xterm'  # Prevent issues with terminal detection
                })
                ...
                if self.deployment_process.returncode == 0:
                    ...
                    self.deploy_status_var.set("Completed")                else:
    ...
    def log_deployment(self, message):
        """Thread-safe deployment logging with proper formatting"""
        def update_log():
            # Enable editing temporarily
            self.deploy_log.config(state='normal')
            ...
            # Add timestamp and insert message
            timestamp = time.strftime("%H:%M:%S")
            self.deploy_log.insert(tk.END, f"[{timestamp}] {clean_message}\n")
            self.deploy_log.see(tk.END)
            
            # Disable editing again to make it truly read-only
            self.deploy_log.config(state='disabled')
