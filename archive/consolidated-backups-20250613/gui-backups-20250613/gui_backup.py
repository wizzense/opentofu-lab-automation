...
class DeploymentManager:
    ...
    def quick_deployment(self):
        ...
        # Start quick deployment        args = ['--quick', '--non-interactive']
        threading.Thread(target=self.run_deployment_command, args=[args], daemon=True).start()
    ...
    def check_prerequisites(self):
        """Check prerequisites without launching external process"""
        self.set_buttons_state(True)
        self.update_status("Checking prerequisites...", "blue")
        self.output_text.delete(1.0, tk.END)
        
        # Perform checks locally instead of launching external process
        def run_checks():
            try:
                ...
                if missing:
                    self.log_output(f"❌ Missing: {', '.join(missing)}")
                else:
        ...
        # Run checks in thread to avoid blocking UI
