...
foreach ($fileName in $targetFiles) {
    ...
    if ($fileName -match '^\d+_(.+)\.Tests\.ps1$') {
        ...
        if ($content -match $pattern) {
            ...
            Write-Host "  ✅ Fixed Get-Command check for $functionName" -ForegroundColor Green
        }
        ...
        if ($content -match $paramPattern) {
            ...
            Write-Host "  ✅ Fixed parameter checks for $functionName" -ForegroundColor Green
        }
    }
    ...
    if ($content -match $dotSourcingPattern) {
        ...
        Write-Host "  ✅ Fixed dot-sourcing syntax check" -ForegroundColor Green
    }
    ...
    if ($content -ne $originalContent) {
        ...
        $fixedCount++
        Write-Host "  ✅ Successfully fixed $fileName" -ForegroundColor Green
    } else {
        Write-Host "  ⏭️  No changes needed for $fileName" -ForegroundColor Yellow
    }
}
    ...
    if ($testFile) {
        ...
        try {
            ...
            if ($result.FailedCount -eq 0 -and $result.PassedCount -gt 0) {
                Write-Host "✅ Test successful: $($result.PassedCount) passed, $($result.FailedCount) failed, $($result.SkippedCount) skipped" -ForegroundColor Green
            } elseif ($result.SkippedCount -gt 0 -and $result.FailedCount -eq 0) {
                Write-Host "⏭️  Tests skipped (platform-specific): $($result.SkippedCount) skipped" -ForegroundColor Yellow
            } else {
                Write-Host "⚠️  Test issues: $($result.PassedCount) passed, $($result.FailedCount) failed, $($result.SkippedCount) skipped" -ForegroundColor Yellow
            }
        } catch {
