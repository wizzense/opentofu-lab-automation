    ...
    try {
        ...
        if (Test-Path $lintScript) {
            ...
            if ($lintCode -eq 0) {
                ...
                $results.TotalErrors++
                Write-Host "❌ Linting completed with errors (code: $lintCode)" -ForegroundColor Red
            }
        } else {
            Write-Host "❌ Lint script not found: $lintScript" -ForegroundColor Red
        ...
        $results.Lint = "ERROR"
        $results.TotalErrors++
        Write-Host "❌ Linting failed with an exception: $_" -ForegroundColor Red
    }
}
...
if (-not $SkipPester) {
    ...
    try {
        ...
        if ($failedTests -eq 0) {
            ...
            Write-Host "✅ Pester tests completed successfully: $passedTests passed, $skippedTests skipped" -ForegroundColor Green
        } else {
            $results.Pester = "FAIL ($failedTests/$totalTests)"
            $results.TotalErrors += $failedTests
            Write-Host "❌ Pester tests completed with $failedTests failures: $passedTests passed, $failedTests failed, $skippedTests skipped" -ForegroundColor Red
        }
    }
    catch {
        $results.Pester = "ERROR"
        $results.TotalErrors++
        Write-Host "❌ Pester tests failed with an exception: $_" -ForegroundColor Red
    }
}
    ...
    try {
        ...
        if (Test-Path $pyTestDir) {
            ...
            if ($pytestCode -eq 0) {
                ...
                $results.TotalErrors++
                Write-Host "❌ Python tests completed with errors (code: $pytestCode)" -ForegroundColor Red
            }
        } else {
            Write-Host "❌ Python test directory not found: $pyTestDir" -ForegroundColor Red
        ...
        $results.TotalErrors++
        Write-Host "❌ Python tests failed with an exception: $_" -ForegroundColor Red
    }
}
    ...
    try {
        ...
        if (Test-Path $healthScript) {
            ...
            if ($healthCode -eq 0) {
                ...
                Write-Host "⚠️ Health check found issues (code: $healthCode)" -ForegroundColor Yellow
            }
        } else {
            Write-Host "❌ Health check script not found: $healthScript" -ForegroundColor Red
        ...
        $results.TotalErrors++
        Write-Host "❌ Health check failed with an exception: $_" -ForegroundColor Red
    }
}
