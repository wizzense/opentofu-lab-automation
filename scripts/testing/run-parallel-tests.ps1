...
Write-Host "⏭️ Skipped:   $totalSkipped" -ForegroundColor Yellow
Write-Host "⏱️ Duration:  $([math]::Round($totalDuration, 2)) seconds" -ForegroundColor Blue
...
if ($Verbosity -eq 'Detailed') {
    ...
    foreach ($result in $totalResults) {
        if ($result.Success) {
            Write-Host "✅ $($result.TestFile): $($result.Passed) passed, $($result.Failed) failed ($([math]::Round($result.Duration, 2))s)" -ForegroundColor Green
        } else {
            Write-Host "❌ $($result.TestFile): ERROR - $($result.Error)" -ForegroundColor Red
        }
    }
}
