    ...
    foreach ($file in $files) {
        ...
        try {
            ...
            if ($emojiMatches.Count -gt 0) {
                ...
                $newContent = $newContent -replace '[\u{2705}]', '[PASS]'  # ✅
                $newContent = $newContent -replace '[\u{274C}]', '[FAIL]'  # ❌
                $newContent = $newContent -replace '[\u{26A0}][\u{FE0F}]?', '[WARN]'  # ⚠️
                $newContent = $newContent -replace '[\u{2139}][\u{FE0F}]?', '[INFO]'  # ℹ️
