...
foreach ($file in $yamlFiles) {
    ...
    if ($fixesApplied.Count -gt 0) {
        Write-Host "✅ Fixed $($fixesApplied.Count) corruptions in: $relativePath" -ForegroundColor Green
...
if ($totalFixed -gt 0) {
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host "✅ No YAML errors found by yamllint" -ForegroundColor Green
        } else {
            $errorCount = ($yamlLintResult | Measure-Object).Count
            Write-Host "⚠️ Found $errorCount YAML issues:" -ForegroundColor Yellow
