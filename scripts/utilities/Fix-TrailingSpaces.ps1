...
foreach ($file in $yamlFiles) {
    ...
    if ($spacesFixed -gt 0) {
        Write-Host "✅ Fixed $spacesFixed trailing spaces in: $relativePath" -ForegroundColor Green
...
if ($totalFixed -gt 0) {
    ...
    try {
        ...
        if ($yamlLintResult) {
            Write-Host "⚠️ Still found trailing space issues:" -ForegroundColor Yellow
            $yamlLintResult | ForEach-Object { Write-Host "  $_" -ForegroundColor Red }
        } else {
            Write-Host "✅ No trailing space errors found by yamllint" -ForegroundColor Green
        }
    }
    catch {
        Write-Host "⚠️ Could not verify with yamllint: $_" -ForegroundColor Yellow
    }
}
