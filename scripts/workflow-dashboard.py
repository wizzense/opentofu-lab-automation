...
def get_workflow_status():
    ...
    if not success:
        print(f"❌ Failed to fetch workflow status: {error}")
        return []
...
def analyze_workflow_files():
    ...
    for workflow_file in workflows_dir.glob('*.yml'):
        try:
            ...
            print(f"⚠️  Failed to analyze {workflow_file}: {e}")
...
def generate_dashboard():
    ...
    for workflow in workflows:
        if workflow['lines'] > 200:
            issues.append(f"⚠️  {workflow['name']} is very long ({workflow['lines']} lines)")
        
        if not workflow['platforms']:
            issues.append(f"⚠️  {workflow['name']} has no platform specified")
...
def main():
    ...
    if not success:
        print("⚠️  GitHub CLI not available - some features will be limited")
