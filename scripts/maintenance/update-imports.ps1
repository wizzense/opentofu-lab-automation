...
foreach ($file in $filesToUpdate) {
    try {
        ...
        $updatedCount++
        Write-Host "  ✅ Updated (backup: $([System.IO.Path]::GetFileName($backupPath)))" -ForegroundColor Green
...
if ($loggerFiles.Count -gt 0) {
    ...
    foreach ($file in $loggerFiles) {
        ...
        try {
            ...
            Write-Host "  ✅ Updated Logger.ps1 reference" -ForegroundColor Green
        } catch {
            Write-Host "  ❌ Failed to update Logger.ps1 reference: $($_.Exception.Message)" -ForegroundColor Red
        }
    }
} else {
    Write-Host "✅ No Logger.ps1 references to update" -ForegroundColor Green
}
