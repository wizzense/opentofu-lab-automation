...
$IssueTrackingPath = Join-Path $ProjectRoot "docs" "reports" "issue-tracking"
...
| **Success Rate** | $($Summary.SuccessRate)% | $(if($Summary.SuccessRate -gt 80) { "✅" } elseif($Summary.SuccessRate -gt 60) { "⚠️" } else { "❌" }) |
| **Total Failures** | $($Summary.TotalFailures) | $(if($Summary.TotalFailures -lt 20) { "✅" } elseif($Summary.TotalFailures -lt 50) { "⚠️" } else { "❌" }) |
| **Skipped Tests** | $($Summary.TotalSkipped) | ℹ️ |
...
- **Prevention Status**: $(if((Run-PreventionChecks).Values -contains $false) { "⚠️ Needs Attention" } else { "✅ Good" })
"@
