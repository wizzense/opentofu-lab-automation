...
class PowerShellExecutor:
    ...
    def __init__(self, project_root: Optional[str] = None):
        ...
        if not self.executor_path.exists():
            raise FileNotFoundError(
                f"CrossPlatformExecutor.ps1 not found at {self.executor_path}"
            )
    
    def encode_script(
        self, script_path: str, parameters: Optional[Dict[str, Any]] = None
    ) -> str:
    ...
    def execute_encoded(
        self, encoded_script: str, whatif: bool = False
    ) -> Dict[str, Any]:
        ...
        result = subprocess.run(cmd, capture_output=True, text=True)
          try:
            return (json.loads(result.stdout) if result.stdout 
                    else {"ExitCode": result.returncode})
    ...
    def validate_encoded(self, encoded_script: str) -> Dict[str, Any]:
        ...
        cmd = [
          ...
          result = subprocess.run(cmd, capture_output=True, text=True)
        return (json.loads(result.stdout) if result.stdout 
                else {"Valid": False, "Error": result.stderr})
