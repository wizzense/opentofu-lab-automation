...
function Invoke-UnifiedMaintenance {
    ...
    if ($UsePatchManager -and (Get-Command Invoke-GitControlledPatch -ErrorAction SilentlyContinue)) {
        ...
        $patchResult = Invoke-GitControlledPatch -PatchDescription "Unified Maintenance: $Mode" -PatchOperation {
            ...
            $results = Invoke-MaintenanceOperations -Mode $Mode :$false # ValidationOnly removed -UpdateChangelog:$UpdateChangelog
            return $results
        } -AutoCommitUncommitted:$false # ValidationOnly removed -TestCommands @(
        ...
        $maintenanceResults.Results = Invoke-MaintenanceOperations -Mode $Mode :$false # ValidationOnly removed -UpdateChangelog:$UpdateChangelog
    }
...
function Invoke-MaintenanceOperations {
    param(
        [string]$Mode,
        [bool]$false # ValidationOnly removed,
    ...
    switch ($Mode) {
        'Quick' {
            $results.Health = Invoke-InfrastructureHealth :$false # ValidationOnly removed
        }
        'Full' {
            $results.Health = Invoke-InfrastructureHealth :$false # ValidationOnly removed
