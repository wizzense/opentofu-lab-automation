...
function Invoke-EnhancedGitOperations {
    ...
    begin {
        ...
        function Invoke-AutomaticValidation {
            Write-Host "SEARCH Running automatic validation..." -ForegroundColor Cyan
            ...
            try {
                ...
                if ($moduleTest) {
                    ...
                    Write-Host "  PASS Module paths accessible" -ForegroundColor Green
                } else {
                    Write-Host "  FAIL Module paths not accessible" -ForegroundColor Red
                }
                ...
                if ($syntaxErrors -eq 0) {
                    ...
                    Write-Host "  WARNING Found $syntaxErrors syntax issues" -ForegroundColor Yellow
                }
                ...
                if ($env:PROJECT_ROOT -and $env:PWSH_MODULES_PATH) {
                    ...
                    Write-Host "  WARNING Environment variables need setup" -ForegroundColor Yellow
                }
                ...
                if ($LASTEXITCODE -eq 0) {
                    ...
                    if ($gitStatus) {
                        ...
                        if ($results.Conflicts.Count -gt 0) {
                            ...
                            Write-Host "  PASS Git status clean" -ForegroundColor Green
                        }
                    } else {
    ...
    process {
        ...
        switch ($Operation) {
            "MergeMain" {
                Write-Host "SYNC Merging with main branch..." -ForegroundColor Yellow
                ...
                if ($LASTEXITCODE -ne 0) {
                    Write-Host "WARNING Merge conflicts detected, attempting resolution..." -ForegroundColor Yellow
                    ...
                    if ($remainingConflicts) {
                        Write-Host "FAIL Some conflicts remain: $($remainingConflicts -join ', ')" -ForegroundColor Red
                        return @{ Success = $false; RemainingConflicts = $remainingConflicts }
                    } else {
                        Write-Host "PASS All conflicts resolved" -ForegroundColor Green
                    }
                }
                
                Write-Host "PASS Merge completed successfully" -ForegroundColor Green
            }
            
            "Stash" {
                Write-Host "[SYMBOL] Stashing changes with cleanup..." -ForegroundColor Yellow
                ...
                if ($LASTEXITCODE -eq 0) {
                    Write-Host "PASS Stash completed successfully" -ForegroundColor Green
                } else {
                    Write-Host "FAIL Stash failed after multiple attempts" -ForegroundColor Red
                    return @{ Success = $false }
                }
            }
            
            "CheckoutMain" {
                Write-Host "SYNC Switching to main branch..." -ForegroundColor Yellow
                ...
                git checkout main
                if ($LASTEXITCODE -eq 0) {
                    Write-Host "PASS Switched to main branch" -ForegroundColor Green
                } else {
                    Write-Host "FAIL Failed to switch to main branch" -ForegroundColor Red
                    return @{ Success = $false }
                }
            }
            
            "ResolveConflicts" {
                Write-Host "TOOL Resolving merge conflicts..." -ForegroundColor Yellow
                
                $conflictFiles = git diff --name-only --diff-filter=U 2>$null
                if (-not $conflictFiles) {
                    Write-Host "PASS No conflicts to resolve" -ForegroundColor Green
                ...
                foreach ($file in $conflictFiles) {
                    ...
                    if ($file -match "\.(md|yml|yaml|json)$") {
                        ...
                        git add "$file"
                        $resolved++
                        Write-Host "    PASS Resolved (kept our version)" -ForegroundColor Green
                    } else {
                        Write-Host "    WARNING Manual resolution needed for: $file" -ForegroundColor Yellow
                    }
                }
                
                Write-Host "PASS Resolved $resolved conflicts automatically" -ForegroundColor Green
            }
        }
        ...
        if ($ValidateAfter) {
            ...
            if ($passed) {
                Write-Host "COMPLETED All validation checks passed!" -ForegroundColor Green
            } else {
                Write-Host "WARNING Some validation checks need attention" -ForegroundColor Yellow
            }
