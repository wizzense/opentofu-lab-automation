...
function Invoke-TieredPesterTests {
    ...
    foreach ($tierName in $tiersToRun) {
        ...
        Write-Host "[SYMBOL] Found $($testFiles.Count) test file(s) for $tierName tier" -ForegroundColor Gray
        ...
        try {
            ...
            if ($Parallel -and $testFiles.Count -gt 1) {
                Write-Host "SYNC Running tests in parallel..." -ForegroundColor Cyan
            ...
            if ($tierFailed -gt 0) {
                if ($shouldBlock) {
                    Write-Host "BLOCKED $tierName tier has failures and is configured to block - stopping execution" -ForegroundColor Red
