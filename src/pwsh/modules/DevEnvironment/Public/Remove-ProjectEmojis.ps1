...
function Remove-ProjectEmojis {
    ...
    begin {        # Import Write-CustomLog if not available
        if (-not (Get-Command Write-CustomLog -ErrorAction SilentlyContinue)) {
            try {
                Import-Module Logging -Force -ErrorAction SilentlyContinue
            } catch {}
        ...
        # Simplified emoji pattern that works reliably in PowerShell
        $emojiPattern = [regex]'[\u2600-\u26FF]|[\u2700-\u27BF]|[\uD800-\uDBFF][\uDC00-\uDFFF]'
