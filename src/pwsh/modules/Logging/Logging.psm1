...
function Start-PerformanceTrace {
    ...
    $script:PerformanceCounters[$OperationName] = @{
        StartTime = Get-Date
        Context = $Context
    }
    
    Write-CustomLog "Performance trace started: $OperationName" -Level TRACE -Context $Context
}
...
function Stop-PerformanceTrace {
    ...
    if (-not $script:LoggingConfig.EnablePerformance -or -not $script:PerformanceCounters.ContainsKey($OperationName)) { 
        return 
    }
    
    $counter = $script:PerformanceCounters[$OperationName]
    $endTime = Get-Date
    ...
    Write-CustomLog "Performance trace completed: $OperationName" -Level TRACE -Context $context
...
function Write-TraceLog {
    ...
    $enhancedContext = $Context + @{
        ...
        Command = if ($caller.InvocationInfo.Line) { $caller.InvocationInfo.Line.Trim() } else { "" }
    }
...
function Write-DebugContext {
    ...
    if ($script:LogLevels[$script:LoggingConfig.LogLevel] -ge $script:LogLevels["DEBUG"]) {
        $caller = Get-PSCallStack | Select-Object -Skip 1 -First 1
        $context.Scope = $Scope
        $context.Function = $caller.FunctionName
        $context.Script = Split-Path $caller.ScriptName -Leaf
    }
    
    Write-CustomLog -Message $Message -Level DEBUG -Context $context
}
