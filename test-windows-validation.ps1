...
# Test 6: Test Windows paths (simulated)
Write-Host "6. Testing Windows Path Handling..." -ForegroundColor Yellow
$windowsPaths = @{
    "C:\temp" = "Temporary directory"
    "C:\HyperV" = "Hyper-V directory"
    "C:\iso_share" = "ISO share directory"
    "C:\Projects" = "Projects directory"
}

foreach ($path in $windowsPaths.Keys) {
    $description = $windowsPaths[$path]
    if ($WhatIf) {
        Write-Host "   What if: Would create $path ($description)" -ForegroundColor Magenta
    } else {
        Write-Host "   ℹ️ Would validate: $path ($description)" -ForegroundColor Cyan
    }
}
